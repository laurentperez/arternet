...
+function ($, socket) {
    ...
    var Ripples = function (el, options) {
        ...
        this.$el.on('mousemove.ripples', function(e) {
            if (that.visible && that.running) that.dropAtMouse(e, that.dropRadius, 0.01, that.from);
        }).on('mousedown.ripples', function(e) {
                if (that.visible && that.running) that.dropAtMouse(e, that.dropRadius * 1.5, 0.14, that.from);
            });
    ...
    Ripples.prototype = {
        ...
        dropAtMouse: function(e, radius, strength, from) {
            this.drop(
        ...
        drop: function(x, y, radius, strength, from) {
            console.log("(js) received drop call from:" + from)
            var that = this;
            var coords = {x:x, y:y, radius:radius, strength:strength, from:from};
            var me = that.from;
            if(from === me) {
            ...
            /*
            if(ua === coords.from) {
                console.log("from myself, no relay")
            } else {
                console.log("relay to other")
